...
class MaterialPurchaseRequisition(models.Model):
    ...
    def confirm_requisition(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup(
                'construction_management.email_employee_purchase_requisition')[1]
            ...
            if template_id:
                values = email_template_obj._generate_template(
                               ...
                               ('subject', 'body_html', 'email_from', 'email_to',
                                'partner_to', 'email_cc', 'reply_to', 'scheduled_date'))[requisition.id]
    ...
    def department_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup(
                'construction_management.email_manager_purchase_requisition')[1]
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj._generate_template([requisition.id],
                                                           ('subject', 'body_html', 'email_from', 'email_to',
                                                            'partner_to', 'email_cc', 'reply_to', 'scheduled_date'))[requisition.id]
    ...
    def action_approve(self):
        for requisition in self:
            ...
            template_id = self.env['ir.model.data']._xmlid_lookup(
                'construction_management.email_user_purchase_requisition')[1]
            email_template_obj = self.env['mail.template'].sudo().browse(template_id)
            if template_id:
                values = email_template_obj._generate_template([requisition.id],
                                                           ('subject', 'body_html', 'email_from', 'email_to',
                            ...
                            if pur_order:
                                ...
                                if requisition.use_manual_locations:
                                    ...
                                    val = {
                                        ...
                                        'location_dest_id': picking_type_id.default_location_dest_id.id or vendor.property_stock_customer.id,
                                        'origin': requisition.sequence
                                    }
